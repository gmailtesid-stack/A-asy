            ...
            <div class="table-responsive">
                <table class="table table-dark-custom mb-0">
                    ...
                    <tbody>
                        ...
                        @endphp
                        <tr>
                            <td><span class="fw-bold text-info">{{ $account->code }}</span></td>
                            <td>{{ $account->name }}</td>
                            <td>
                                <span class="badge bg-{{ $account->type == 'asset' ? 'primary' : ($account->type == 'revenue' ? 'success' : 'warning') }}-soft">
                                    {{ strtoupper($account->type) }}
                                </span>
                            </td>
                            <td class="text-end">Rp {{ number_format($account->total_debit ?? 0, 0, ',', '.') }}</td>
                            <td class="text-end">Rp {{ number_format($account->total_credit ?? 0, 0, ',', '.') }}</td>
                            <td class="text-end fw-bold {{ $balance < 0 ? 'text-danger' : 'text-white' }}">
