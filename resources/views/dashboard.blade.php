        ...
        <div class="card border-0 overflow-hidden shadow-sm" style="background: linear-gradient(135deg, #6366f1 0%, #4f46e5 100%);">
            <div class="card-body p-4 p-md-5 text-white position-relative">
                <div class="position-relative z-index-1">
                    <h2 class="fw-800 mb-2">Selamat Datang Kembali, {{ explode(' ', auth()->user()->name)[0] }}! 👋</h2>
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        @if(auth()->user()->isCashier() || auth()->user()->isManager() || auth()->user()->isSuperAdmin())
                            <a href="{{ route('pos.index') }}" class="btn btn-light fw-bold px-4 py-2 rounded-pill shadow-sm">
                                <i class="bi bi-cart-plus-fill me-2"></i> Buka Kasir
                            </a>
                        @endif
                        @if(!auth()->user()->isCashier())
                            <a href="{{ route('reports.index') }}" class="btn btn-outline-light fw-bold px-4 py-2 rounded-pill">
    ...
    <div class="col-md-4">
        <div class="card border-0 h-100 shadow-sm hover-up">
            <div class="card-body p-4 text-center">
                ...
                <a href="{{ route('outlets.index') }}" class="btn btn-sm btn-primary px-3 rounded-pill">Kelola</a>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-4">
        <div class="card border-0 h-100 shadow-sm hover-up">
            <div class="card-body p-4 text-center">
                ...
                <a href="{{ route('products.index') }}" class="btn btn-sm btn-success px-3 rounded-pill">Cek Stok</a>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-4">
        <div class="card border-0 h-100 shadow-sm hover-up">
            <div class="card-body p-4 text-center">
                ...
                <a href="{{ route('users.index') }}" class="btn btn-sm btn-info px-3 rounded-pill text-white">Atur Tim</a>
            </div>
        </div>
    </div>
</div>
